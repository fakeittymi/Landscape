...
namespace WindowsFormsApplication317
{
    public partial class Form1 : Form
    {
        ...
        //HeightMap map = new HeightMap(1000);
        ...
        private void Render(Matrix4x4 worldMatrix)
        {
            using (var wr = new ImageWrapper(result))
                foreach (var v in voxels)
                {
                    ...
                    var intY = (int)p.Y;
                    //цвет
                    Color color = Color.FromArgb(50, 200, 30);
                    if (v.Light > -1 && v.Light < 17)                   
                        color = Color.FromArgb(0, 0, 255);
                    if (v.Light > 16 && v.Light < 33)
                        color = Color.FromArgb(0, 64, 255);
                    if (v.Light > 32 && v.Light < 49)
                        color = Color.FromArgb(0, 128, 255);
                    if (v.Light > 48 && v.Light < 65)
                        color = Color.FromArgb(0, 196, 255);
                    if (v.Light > 64 && v.Light < 81)
                        color = Color.FromArgb(0, 255, 255);
                    if (v.Light > 80 && v.Light < 97)
                        color = Color.FromArgb(0, 255, 196);
                    if (v.Light > 96 && v.Light < 113)
                        color = Color.FromArgb(0, 255, 128);
                    if (v.Light > 112 && v.Light < 128)
                        color = Color.FromArgb(0, 255, 64);
                    if (v.Light > 128 && v.Light < 145)
                        color = Color.FromArgb(0, 200, 0);
                    if (v.Light > 144 && v.Light < 161)
                        color = Color.FromArgb(64, 200, 0);
                    if (v.Light > 160 && v.Light < 177)
                        color = Color.FromArgb(128, 255, 0);
                    if (v.Light > 176 && v.Light < 193)
                        color = Color.FromArgb(196, 255, 0);
                    if (v.Light > 192 && v.Light < 209)
                        color = Color.FromArgb(255, 255, 0);
                    if (v.Light > 208 && v.Light < 225)
                        color = Color.FromArgb(255, 196, 0);
                    if (v.Light > 224 && v.Light < 241)
                        color = Color.FromArgb(255, 128, 0);
                    if (v.Light > 240 && v.Light < 257)
                        color = Color.FromArgb(255, 64, 0);

                    //color = Color.FromArgb(v.Light, v.Light, 0);
